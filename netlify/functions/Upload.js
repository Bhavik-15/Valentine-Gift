...
if (!GITHUB_TOKEN) {
return { statusCode: 500, body: JSON.stringify({ error: ‘Server not configured — GITHUB_TOKEN missing in Netlify env vars’ }) };
}
...
return { statusCode: 400, body: JSON.stringify({ error: ‘filePath and fileContent required’ }) };
}
try {
...
```
  ...
  if (!putRes.ok) {
    ...
    return { statusCode: putRes.status, body: JSON.stringify({ error: err.message || 'GitHub API error' }) };
  }
...
if (!r.ok) return { statusCode: r.status, body: JSON.stringify({ error: ‘Could not read data.js’ }) };
const d = await r.json();
return { statusCode: 200, body: JSON.stringify({ content: d.content, sha: d.sha }) };
} catch (err) {
return { statusCode: 500, body: JSON.stringify({ error: err.message }) };
}
}
